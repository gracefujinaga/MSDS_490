...
It seems like it is better to not normalize the data for this dataset. Here, normalziation refers to centering each row by subtracting the user's mean rating from each rating. All 3 metrics decreased across the board, and it looks like normalizing may have taken out some salient features from this dataset.
